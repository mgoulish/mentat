...
parent     = r'parent=([^ ]+)'
brackets   = r'\[([^ ]+)\]'
ip_addr    = r'(\d+\.\d+\.\d+\.\d+:\d+)'    # 254.12.22.23:55671
to_addr    = r'to (:\d+)'
message    = r'(.*?)'
...
def parse_unknown_protocol ( log_line ) :
  ...
  global matched
  pattern = date + ' ' + time + skip + brackets + skip + parent + skip + brackets + " Connection from " + ip_addr + skip + to_addr
...
def parse_no_route_to_host ( log_line ) :
  ...
  global matched
  matched += 1
  pattern = date_time + skip + brackets + skip + parent + skip + brackets + skip + "Connection to " + ip_addr + message + " sourceFile"
  match = re.match ( pattern, log_line)
  event = new_event ( )
  event['type']         = 'connection_failed'
  event['timestamp']    = match.group(1) + ' ' + match.group(2)
...
def parse_connection_timed_out ( log_line ) :
  ...
  global matched
  #print ( f"match: connection timed out |{log_line}|" )
  matched += 1
  event = new_event ( )
...
def parse_BEGIN_END_line ( log_line ) :
  ...
  print ( f"parse_BEGIN_END_line error: unmatched line: |{log_line}|" )
  unmatched += 1
...
def find_begin_end_lines ( sites ) :
  ...
  print ( "\n\n\nmain:" )
  print ( f"matched line count   : {matched}" )
  print ( f"unmatched line count : {unmatched}" )
